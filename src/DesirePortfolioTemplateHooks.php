 ...
 */
class DesirePortfolioTemplateHooks {

	static function get_portfolio_filters() {
	...
	static function get_portfolio_projects() {
		...
		$args = array(
			'post_type'      => array( 'jetpack-portfolio', 'desire-portfolio' ),
			'paged'          => $paged,
		...
		if ( ( post_type_exists( 'jetpack-portfolio' ) || post_type_exists( 'desire-portfolio' ) ) && $project_query->have_posts() ) :

			while ( $project_query->have_posts() ) : $project_query->the_post();
				$taxonomies = wp_get_post_terms( get_the_ID(), array(
					'jetpack-portfolio-type',
					'desire-portfolio-type'
				) );
				$types      = "";
				foreach ( $taxonomies as $type ) {
					$types .= "type-" . $type->slug . " ";
				}
				$types = substr( $types, 0, -1 );
				?>
				<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
					<div class="portfolio-item <?php echo $types; ?>">
